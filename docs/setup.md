...
The goal for `rahuldhole.com` is to serve as the main portfolio and blog hub. Future apps or specific tools currently shown in the projects section (like HTML Playground IDE) will be migrated to subdomains like `html-playground.rahuldhole.com`.

### How to set up Subdomains:

1. **DNS Provider (e.g., Cloudflare)**:
   - Add a new `A` or `CNAME` record for your subdomain (e.g., `NAME: html-playground`, `TARGET: <your server IP or Vercel edge url>`).
2. **Hosting Platform (e.g., Vercel / Netlify / Self-hosted)**:
   - Go to your Project Settings.
   - Under Domains, add `html-playground.rahuldhole.com`.
