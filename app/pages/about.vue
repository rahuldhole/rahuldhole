...
<template>
  <UPage v-if="page">
    <UPageHero :title="page.title" :description="page.description" orientation="horizontal" :ui="{
      ...
      <img src="/profile.png" alt="Rahul Dhole" class="sm:rotate-4 size-36 rounded-full ring ring-default ring-offset-3 ring-offset-(--ui-bg) object-cover" />
    </UPageHero>
    <UPageSection :ui="{
      container: '!pt-0'
    }">
      ...
      <div class="flex flex-row justify-center items-center py-10 space-x-[-2rem]">
        <Motion v-for="(image, index) in page.images" :key="index"
          :initial="{ opacity: 0, scale: 0.9, rotate: (index % 2 === 0 ? -6 : 6) }"
          :while-in-view="{ opacity: 1, scale: 1, rotate: (index % 2 === 0 ? -3 : 3) }"
          :transition="{ delay: 0.2 * index }" :in-view-options="{ once: true }">
