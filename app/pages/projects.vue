...
<template>
  <UPage v-if="page">
    ...
    >
      <div v-if="projects" class="grid grid-cols-12 gap-4 md:gap-5 lg:gap-6 auto-rows-min">
        <!-- Slot 1: Large Typography / Intro Project -->
        <div v-if="regularProjects[0]" class="col-span-12 md:col-span-6 lg:col-span-5 group relative bg-gray-50 dark:bg-zinc-900/50 rounded-[2rem] lg:rounded-[2.5rem] p-6 md:p-8 lg:p-10 flex flex-col justify-between border border-transparent hover:border-primary/20 transition-all duration-500 min-h-[250px]">
          <div class="flex justify-between items-start">
            <h3 class="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-gray-900 dark:text-white uppercase leading-[0.9]">
          ...
          <div class="mt-6 md:mt-8">
            <p class="text-gray-500 dark:text-gray-400 text-base md:text-lg leading-relaxed max-w-xs mb-4 md:mb-6">
        ...
        <div v-if="regularProjects[1]" class="col-span-12 md:col-span-6 lg:col-span-3 group relative bg-white dark:bg-zinc-800 rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden flex flex-col border border-gray-100 dark:border-zinc-800 shadow-sm transition-all hover:shadow-xl">
          <div class="p-6 md:p-8 pb-0">
            <h4 class="text-lg md:text-xl font-bold uppercase tracking-tighter mb-1">{{ regularProjects[1].title }}</h4>
            <span class="text-[10px] font-bold text-primary tracking-widest uppercase">QUICK LOOK</span>
          </div>
          <div class="flex-1 flex items-center justify-center p-4 md:p-6 relative">
            <img :src="regularProjects[1].image" :alt="regularProjects[1].title + ' project preview'" class="w-full h-40 md:h-48 object-contain transition-transform duration-700 group-hover:scale-110 drop-shadow-2xl" />
        ...
        <div v-if="regularProjects[2]" class="col-span-12 md:col-span-6 lg:col-span-4 lg:row-span-2 group relative bg-zinc-950 text-white rounded-[2rem] lg:rounded-[3rem] p-6 md:p-8 lg:p-10 flex flex-col justify-between overflow-hidden min-h-[300px]">
          <div class="relative z-10">
            <div class="flex items-center gap-3 mb-4 md:mb-6">
              <div class="p-2 bg-primary/20 rounded-lg">
                <UIcon name="i-lucide-cpu" class="text-primary size-5 md:size-6" />
              </div>
              <span class="text-xs font-mono text-primary/80 tracking-widest">SYSTEM_VERSION_1.0</span>
            </div>
            <h3 class="text-2xl md:text-3xl font-bold leading-tight mb-3 md:mb-4">{{ regularProjects[2].title }}</h3>
            <p class="text-zinc-400 text-sm leading-relaxed mb-4 md:mb-6">{{ regularProjects[2].description }}</p>
          </div>
        ...
        <div v-if="regularProjects[3]" class="col-span-12 md:col-span-6 lg:col-span-3 group relative bg-zinc-900 text-white rounded-[2rem] lg:rounded-[2.5rem] p-6 md:p-8 flex flex-col items-center justify-center text-center border border-zinc-800 transition-all hover:bg-zinc-800 min-h-[200px]">
          <div class="mb-4">
             <span class="text-4xl md:text-5xl font-bold tracking-tighter">98<span class="text-primary">%</span></span>
        ...
        <div v-if="regularProjects[4]" class="col-span-12 lg:col-span-5 group relative bg-white dark:bg-zinc-900 rounded-[2rem] lg:rounded-[3rem] overflow-hidden border border-gray-100 dark:border-zinc-800 flex flex-col md:flex-row transition-all hover:shadow-2xl">
          <div class="p-6 md:p-10 flex flex-col justify-center md:max-w-sm shrink-0 relative z-10">
            <span class="text-[10px] font-bold text-primary tracking-[0.2em] mb-3 md:mb-4 uppercase">CORE DEVELOPMENT</span>
            <h3 class="text-2xl md:text-3xl font-bold mb-3 md:mb-4 tracking-tight leading-none">{{ regularProjects[4].title }}</h3>
        ...
        <div v-if="regularProjects[5]" class="col-span-12 md:col-span-6 lg:col-span-4 group relative bg-gray-50 dark:bg-zinc-900 border border-transparent hover:border-primary/30 rounded-[2rem] lg:rounded-[2.5rem] p-6 md:p-8 flex items-center justify-center transition-all duration-500 min-h-[180px]">
           <div class="text-center group-hover:scale-105 transition-transform duration-500">
             <div class="flex items-center justify-center gap-3 mb-4">
               <UIcon name="i-lucide-zap" class="size-6 md:size-8 text-primary shadow-lg shadow-primary/20" />
               <h4 class="text-xl md:text-2xl font-black italic tracking-tighter uppercase">{{ regularProjects[5].title }}</h4>
             </div>
        ...
        <div v-if="streakProject" class="col-span-12 md:col-span-12 lg:col-span-5 group relative bg-[#F8FAFC] dark:bg-zinc-900/40 rounded-[2rem] lg:rounded-[2.5rem] p-6 md:p-10 flex flex-col justify-between border border-transparent hover:border-primary/20 transition-all duration-500 overflow-hidden">
          <div class="flex flex-col md:flex-row md:items-center justify-between relative z-10 gap-4">
            <div class="flex flex-col">
              ...
              <h3 class="text-2xl md:text-3xl font-black italic tracking-tighter text-gray-900 dark:text-white uppercase leading-none">
                {{ streakProject.title }}
              </h3>
            </div>
            <div class="flex items-center gap-2">
              <UIcon name="i-lucide-flame" class="size-6 md:size-8 text-orange-500 animate-pulse" />
              <span class="text-xl md:text-2xl font-bold font-mono">STREAK</span>
            </div>
          </div>

          <div class="my-4 md:my-6 relative z-10 flex justify-center">
          ...
          <div class="flex flex-col md:flex-row justify-between items-start md:items-end relative z-10 gap-4">
            <div class="flex flex-col">
            ...
            <div class="md:text-right">
              <p class="text-xs text-gray-500 dark:text-gray-400 font-medium md:max-w-[200px] mb-2">{{ streakProject.description }}</p>
        ...
        <template v-for="(project, index) in regularProjects.slice(7)" :key="project.path">
           <div class="col-span-12 md:col-span-6 lg:col-span-4 group relative bg-white dark:bg-zinc-900 rounded-[1.5rem] md:rounded-[2rem] p-5 md:p-6 border border-gray-100 dark:border-zinc-800 transition-all hover:shadow-xl">
             ...
             <p class="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-3 md:mb-4">{{ project.description }}</p>
             <div class="flex flex-wrap gap-2">
